...
export default function AdminDisputesPage() {
  ...
  const fetchDisputes = async () => {
    try {
      ...
      console.error('Failed to fetch disputes from backend', e);
    } finally {
      setLoading(false);
    }
  };
  ...
  const filteredDisputes = useMemo(() => {
    return disputesList.filter((d: any) => {
  ...
  const handleResolve = (id: string) => {
    ...
    setTimeout(() => {
      setData((prev: any) => ({
        ...prev,
        disputes: prev?.disputes?.map((d: any) => d.id === id ? { ...d, status: 'RESOLVED_REFUNDED' } : d)
      }));
      setResolvingId(null);
    }, 600);
  };
    ...
    <div className="space-y-6 pb-16">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
        {[
          ...
          { label: 'Disputed Volume', val: `₹${(summary.disputed_volume_inr || 119992).toLocaleString('en-IN')}`, change: '0.012% of GMV', color: 'text-blue-600' },
      ...
      <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-2xs flex flex-col md:flex-row items-center justify-between gap-4">
        ...
        <div className="flex flex-wrap items-center gap-1 bg-slate-100 p-1 rounded-xl text-xs font-semibold text-slate-600">
          {['ALL', 'UNDER_ARBITRATION', 'EVIDENCE_SUBMITTED', 'RESOLVED_REFUNDED', 'AUTO_RESOLVED'].map((st) => (
            <button
              key={st}
                ...
                {filteredDisputes.length === 0 ? (
                  <tr>
                    <td colSpan={8} className="py-12 text-center space-y-3">
                      ...
                      <p className="text-xs text-slate-500">No disputes matching "{searchQuery}".</p>
                    </td>
                  </tr>
                ) : (
                  filteredDisputes.map((d: any) => (
                    <tr key={d.id} className="hover:bg-slate-50/60 transition-colors">
                      ...
                      <td className="py-3.5 px-4 font-mono text-[11px] text-slate-500">
                        {d.sla_hours_remaining ? `${d.sla_hours_remaining}h left` : d.raised_at}
                      </td>
