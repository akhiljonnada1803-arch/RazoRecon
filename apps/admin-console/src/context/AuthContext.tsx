...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    try {
      const savedToken = localStorage.getItem('razorcommerce_admin_token') || localStorage.getItem('razorcommerce_token');
      const savedUser = localStorage.getItem('razorcommerce_admin_user') || localStorage.getItem('razorcommerce_user');

      if (savedToken && savedUser) {
        const parsed = JSON.parse(savedUser);
        if (parsed.role === 'Platform Admin' || parsed.role_id === 'role_platform_admin') {
          setToken(savedToken);
          setUser(parsed);
        } else {
          setToken(null);
  ...
  const login = async (email: string, password: string): Promise<boolean> => {
    try {
      ...
      if (resp && resp.access_token) {
        ...
        setUser(resp.user);
        localStorage.setItem('razorcommerce_admin_token', resp.access_token);
        localStorage.setItem('razorcommerce_admin_user', JSON.stringify(resp.user));
  ...
  const logout = () => {
    setToken(null);
    setUser(null);
    localStorage.removeItem('razorcommerce_admin_token');
    localStorage.removeItem('razorcommerce_admin_user');
