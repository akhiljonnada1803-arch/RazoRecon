...
export default function DemandIntelligencePage() {
  ...
  const [activeTab, setActiveTab] = useState<'ALL' | 'TRENDING' | 'DECLINING' | 'DEAD' | 'CAMPAIGNS'>('ALL');
  ...
  const filteredProducts = products.filter(p => {
    ...
    if (activeTab === 'DECLINING') return p.demand_score >= 20 && p.demand_score < 40;
    if (activeTab === 'DEAD') return p.demand_score < 20;
  ...
  return (
    <div className="space-y-8 pb-20">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200 pb-3">
        <div className="flex items-center gap-2 overflow-x-auto pb-1 scrollbar-none">
          {[
            ...
            { key: 'TRENDING', label: `🔥 Trending & Growing (${trending.length + (data?.growing_products?.length || 0)})` },
            { key: 'DECLINING', label: `📉 Declining (${declining.length})` },
            { key: 'DEAD', label: `💀 Dead Inventory (${deadInventory.length})` },
      ...
      {/* Section 3: Declining & Dead Inventory Highlight Cards */}
      {(activeTab === 'ALL' || activeTab === 'DECLINING' || activeTab === 'DEAD') && (
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="font-bold text-base text-slate-900 flex items-center gap-2">
                <Sparkles className="h-4 w-4 text-amber-500" />
                <span>AI Dynamic Discount & Liquidation Recommendations</span>
              </h2>
              <p className="text-xs text-slate-500">
                Actionable markdown proposals for declining and dead inventory to restore cash velocity.
              </p>
            </div>
          </div>
                  ...
                  <tr key={p.id} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="py-3.5 px-4 text-right">
                      {p.ai_recommendation ? (
                        ...
                        <Link href="/merchant/inventory-optimization">
                          <Button
                            size="sm"
                            variant="outline"
