...
export function AuditActivityFeed({ limit = 15, title = "Recent System Audit Feed", showFilters = true }: { limit?: number; title?: string; showFilters?: boolean }) {
  ...
  const { data, isLoading, refetch, isFetching, dataUpdatedAt } = useQuery<{ items: AuditLog[]; total: number }>({
    queryKey: ['audit-activity-feed', limit],
    queryFn: async () => {
  ...
  const getRoleBadge = (role: string) => {
    ...
    if (role === 'Platform Admin' || role === 'Super Admin') {
      return <Badge className="bg-slate-900 text-amber-300 font-mono text-[10px]">👑 Admin</Badge>;
    }
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between pb-3 border-b border-slate-100 gap-3">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500">Immutable ISO 8601 UTC audit trails & business action logs</p>
          </div>
        </div>
              ...
              >
                ...
                <div className="flex items-center justify-between text-slate-500 text-[11px]">
                  ...
                  {hasDiff && (
                    ...
                    >
                      <span>{isExpanded ? 'Hide Changes' : 'View Diff'}</span>
