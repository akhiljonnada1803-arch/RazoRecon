...
interface VendorIntelligenceDirectoryTableProps {
  vendors: VendorRiskScoreDTO[];
  onSelectVendor: (vendor: VendorRiskScoreDTO) => void;
}

export const VendorIntelligenceDirectoryTable: React.FC<VendorIntelligenceDirectoryTableProps> = ({
  vendors,
  onSelectVendor,
}) => {
      ...
      <CardHeader className="p-4 pb-3 border-b border-slate-100 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          ...
          <p className="text-xs text-slate-500">
            Click any row to open the forensic counterparty dossier, pattern memory, and resolution history
          </p>
        </div>
        ...
        <div className="flex items-center gap-2">
          <div className="relative">
            ...
            <Input
              placeholder="Search counterparty or issue..."
              value={search}
              onChange={(e) => setSearch(e.target.value)}
              className="h-8 pl-8 text-xs w-48 sm:w-60 border-slate-200"
            />
          </div>
      ...
      <CardContent className="p-0 overflow-x-auto">
        <table className="w-full text-left text-xs">
          <thead className="bg-slate-50/80 text-slate-500 font-semibold border-b border-slate-100">
            <tr>
              ...
              <th className="py-2.5 px-3">EXCEPTIONS / TXNS</th>
                ...
                >
                  <td className="py-3 px-4">
                    <div className="flex items-center gap-2.5">
                      <div className="h-7 w-7 rounded bg-slate-100 border border-slate-200 flex items-center justify-center text-slate-600 font-bold text-[10px] shrink-0 group-hover:border-[#0B72E7] group-hover:text-[#0B72E7] transition-colors">
                        {v.vendor.slice(0, 2).toUpperCase()}
                      </div>
                      <div>
                        <span className="font-bold text-slate-900 group-hover:text-[#0B72E7] transition-colors block">
                          {v.vendor}
                        </span>
                  ...
                  <td className="py-3 px-3">
                    <div className="flex items-center gap-1.5">
                      <span className={`font-mono font-bold text-sm ${isHigh ? 'text-rose-600' : isMed ? 'text-amber-600' : 'text-emerald-600'}`}>
                        {v.risk_score}
                      </span>
                  ...
                  <td className="py-3 px-3">
                    ...
                    <span className="text-[10px] text-slate-400">
                      {v.duplicate_payment_count > 0 ? `${v.duplicate_payment_count} Dups` : `${v.settlement_delay_count} Delays`}
                    </span>
                  </td>

                  <td className="py-3 px-3">
                    <span className={`font-mono font-bold text-xs ${v.total_exceptions > 0 ? 'text-amber-600' : 'text-slate-800'}`}>
                      {v.total_exceptions} Excs
                    </span>
                  ...
                  <td className="py-3 px-4 text-right">
                    <Button
                      variant="ghost"
                      size="sm"
                      className="h-7 text-xs px-2 rounded group-hover:bg-white group-hover:text-[#0B72E7] group-hover:shadow-2xs text-slate-500 font-medium gap-1"
                    >
        ...
        {filtered.length === 0 && (
          <div className="p-8 text-center text-slate-500">
            No counterparties found matching search criteria.
          </div>
        )}
      </CardContent>
    </Card>
